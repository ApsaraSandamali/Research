...
def transportation_simplex_method(supply, demand, costs, penalties=None):
    ...
    basic_variables = inner(north_west_corner(
...
def write_output(solution,cost,time,i):
    with open(f"{os.getcwd()}/or_output/output{i}"+".txt", "w") as f:
        f.write(f"Solution: \n {solution}")
        f.write("\n\n")
        f.write(f"Total cost: {cost}")
        f.write("\n")
        f.write(f"Total runtime: {time}" )
...
#     [5, 5, 5, 5, 5, 5, 5, 5, 5, 1]
# ]
# supply = [1, 2, 1, 1, 1, 2, 1, 1, 1, 2, 3, 2, 1, 4, 1, 1, 1, 1, 1, 2]
# demand = [4, 4, 2, 5, 2, 3, 2, 2, 2, 4]
...
# write_output(solution, total_cost, f"{total_time} ms")